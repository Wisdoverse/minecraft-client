...
if (!params.type) {
  console.error(JSON.stringify({
    success: false,
    error: 'Missing required parameter: --type is required (entities/blocks/chat)'
  }, null, 2));
  process.exit(1);
}
...
// Validate type
const validTypes = ['entities', 'blocks', 'chat'];
if (!validTypes.includes(params.type)) {
  console.error(JSON.stringify({
    success: false,
    error: 'Invalid type: ' + params.type + '. Supported types: entities, blocks, chat'
  }, null, 2));
  process.exit(1);
}
...
}, 10000); // Longer timeout for block scanning
